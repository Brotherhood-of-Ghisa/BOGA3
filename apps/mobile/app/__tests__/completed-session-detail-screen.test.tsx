...
jest.mock("@/src/data", () => ({
  ...
  listSessionListBuckets: jest
    .fn()
    .mockResolvedValue({ active: null, completed: [] }),
...
let mockExerciseCatalogState = {
  ...
  muscleGroups: [
    { id: "chest", displayName: "Chest", familyName: "Chest", sortOrder: 1 },
  ],
  muscleGroupsById: {},
...
const { captureRef: mockCaptureRef, releaseCapture: mockReleaseCapture } =
  jest.requireMock("react-native-view-shot") as {
    captureRef: jest.Mock;
    releaseCapture: jest.Mock;
  };
...
const COMPLETED_SESSION_DETAIL_FIXTURE: CompletedSessionDetailRecord = {
  ...
  exercises: [
    ...
    },
    {
      ...
      machineName: "Cable",
      tags: [],
      sets: [{ id: "set-5", weight: "120", reps: "12", setType: null }],
    },
  ],
};
...
describe("CompletedSessionDetailScreenShell", () => {
  ...
  it("validates completion presentation route values", () => {
    expect(resolveCompletedSessionPresentation("completion")).toBe(
      "completion",
    );
    expect(resolveCompletedSessionPresentation(["completion"])).toBe(
      "completion",
  ...
  it("renders the no-PR completion hierarchy and hides ordinary detail actions", async () => {
    const dataClient: CompletedSessionDetailDataClient = {
      loadCompletedSession: jest
        .fn()
        .mockResolvedValue(COMPLETED_SESSION_DETAIL_FIXTURE),
      loadInsights: jest.fn().mockResolvedValue({
        personalRecords: [],
        exerciseVolumeComparisons: [],
      }),
      appendCompletedSessionExerciseAsPlanned: jest
        .fn()
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("session-completion-presentation"),
      ).toBeTruthy();
    });
    expect(mockStackScreen).toHaveBeenCalledWith(
      expect.objectContaining({
        options: expect.objectContaining({ title: "Session complete" }),
      }),
    );
    expect(screen.getByTestId("session-completion-duration")).toHaveTextContent(
      "58 min",
    );
    expect(
      screen.getByTestId("session-completion-exercises"),
    ).toHaveTextContent("2");
    expect(screen.getByTestId("session-completion-sets")).toHaveTextContent(
      "5 (3 working)",
    );
    ...
    expect(
      screen.queryByTestId("session-completion-personal-records"),
    ).toBeNull();
    expect(
      screen.getByTestId("session-completion-muscle-chest"),
    ).toHaveTextContent("Chest (3)");
    expect(
      screen.queryByTestId("session-completion-view-muscle-load"),
    ).toBeNull();
    expect(
      screen.queryByTestId("completed-session-detail-action-bar"),
    ).toBeNull();
  ...
  it("keeps completion and current exercise rows available when optional insight history fails", async () => {
    const dataClient: CompletedSessionDetailDataClient = {
      ...
      loadInsights: jest
        .fn()
        .mockRejectedValue(new Error("Insight history unavailable")),
      appendCompletedSessionExerciseAsPlanned: jest
        .fn()
        .mockResolvedValue(undefined),
      setCompletedSessionDeletedState: jest.fn().mockResolvedValue(undefined),
    ...
    expect(
      screen.getByTestId("session-completion-exercise-exercise-1"),
    ).toBeTruthy();
  ...
  it("omits an empty muscle breakdown without removing the completion exits", async () => {
    ...
    expect(
      screen.queryByTestId("session-completion-muscle-breakdown"),
    ).toBeNull();
    expect(
      screen.queryByTestId("session-completion-view-muscle-load"),
    ).toBeNull();
  ...
  it("keeps the Maestro-only catalog failure state informational and non-interactive", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("session-completion-muscle-empty-state"),
      ).toHaveTextContent("Muscle breakdown unavailable.");
    });
  ...
  it("shows every PR at once and previews the complete session image", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("session-completion-pr-bench-press"),
      ).toBeTruthy();
      expect(
        screen.getByTestId("session-completion-pr-lat-pulldown"),
      ).toBeTruthy();
    });
    ...
    expect(
      screen.getByTestId("session-completion-exercise-exercise-2"),
    ).toBeTruthy();
    ...
    expect(
      screen.getByTestId("session-share-card-pr-bench-press"),
    ).toBeTruthy();
    expect(
      screen.getByTestId("session-share-card-pr-lat-pulldown"),
    ).toBeTruthy();
    expect(
      screen.getByTestId("session-share-exercise-exercise-1"),
    ).toBeTruthy();
    expect(
      screen.getByTestId("session-share-exercise-exercise-2"),
    ).toBeTruthy();
    expect(
      within(screen.getByTestId("session-share-card")).queryByText(
        "Westside Barbell Club",
      ),
    ).toBeNull();
    expect(
      screen.getByText("Nothing is shared until you choose an app."),
    ).toBeTruthy();
  });
  ...
  it("keeps session image-share failures inline and retries the same complete preview", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId("session-completion-share-session"),
      ).toBeTruthy(),
    );
    fireEvent.press(screen.getByTestId("session-completion-share-session"));
  ...
  it("renders loading then a recorder-like read-only detail on success", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("completed-session-detail-screen"),
      ).toBeTruthy();
    });
    ...
    expect(
      screen.queryByTestId("completed-session-detail-reopen-button"),
    ).toBeNull();
    expect(screen.getAllByText("Append")).toHaveLength(2);
    expect(screen.getByText("Delete")).toBeTruthy();
    expect(
      screen.getByTestId("completed-session-detail-screen").props
        .stickyHeaderIndices,
    ).toEqual([0]);
    expect(
      screen.getByTestId(
        "completed-session-detail-sets-table-header-exercise-1",
      ),
    ).toBeTruthy();
    ...
    expect(
      screen.getByTestId("completed-session-detail-tags-exercise-1"),
    ).toBeTruthy();
  ...
  it("does not render tag chips when an exercise has no assigned tags", async () => {
    const dataClient: CompletedSessionDetailDataClient = {
      loadCompletedSession: jest.fn().mockResolvedValue({
        ...COMPLETED_SESSION_DETAIL_FIXTURE,
        exercises: COMPLETED_SESSION_DETAIL_FIXTURE.exercises.map(
          (exercise) => ({
            ...exercise,
            tags: [],
          }),
    ...
    expect(
      screen.queryByTestId("completed-session-detail-tags-exercise-1"),
    ).toBeNull();
  });
  ...
  it("collapses each exercise to valid performed-set and working-set counts", async () => {
    ...
    expect(
      screen.getByTestId(
        "completed-session-detail-collapsed-summary-exercise-1-counts",
      ),
    ).toHaveTextContent("4 sets · 3 w/sets");
    expect(
      screen.queryByTestId(
        "completed-session-detail-collapsed-summary-exercise-1-new-pr",
      ),
    ).toBeNull();
    expect(
      screen.queryByTestId(
    ...
    expect(
      screen.getByTestId(
        "completed-session-detail-append-exercise-button-exercise-1",
      ),
    ).toBeTruthy();
  ...
  it("per-exercise append action calls the data client and opens the active session", async () => {
    const mockAppendCompletedSessionExercise = jest
      .fn()
      .mockResolvedValue({ sessionId: "active-1" });
    const dataClient: CompletedSessionDetailDataClient = {
      loadCompletedSession: jest.fn().mockResolvedValue({
        ...COMPLETED_SESSION_DETAIL_FIXTURE,
      }),
      appendCompletedSessionExerciseAsPlanned:
        mockAppendCompletedSessionExercise,
      setCompletedSessionDeletedState: jest.fn().mockResolvedValue(undefined),
    };
    ...
    await waitFor(() => {
      expect(mockAppendCompletedSessionExercise).toHaveBeenCalledWith(
  ...
  it("renders one append action for each exercise block", async () => {
    ...
    expect(
      screen.getByTestId(
        "completed-session-detail-append-exercise-button-exercise-2",
      ),
    ).toBeTruthy();
    expect(
      screen.getByLabelText("Append Bench Press block to current session"),
    ).toBeTruthy();
    expect(
      screen.getByLabelText("Append Lat Pulldown block to current session"),
    ).toBeTruthy();
  });
  ...
  it("shows non-destructive feedback when append fails", async () => {
    const dataClient: CompletedSessionDetailDataClient = {
      ...
      appendCompletedSessionExerciseAsPlanned: jest
        .fn()
        .mockRejectedValue(new Error("Unable to append now")),
      setCompletedSessionDeletedState: jest.fn().mockResolvedValue(undefined),
    };
  ...
  it("reloads the completed session when the detail screen regains focus", async () => {
    ...
    const { __triggerFocus: triggerFocus } = jest.requireMock(
      "expo-router",
    ) as {
      __triggerFocus: () => void;
    };
  ...
  it("delete and undelete persist through the data client and update the action label", async () => {
    ...
    expect(screen.getByText("Delete")).toBeTruthy();
    fireEvent.press(
      screen.getByTestId("completed-session-detail-delete-button"),
    );
    await waitFor(() => {
      expect(mockSetCompletedSessionDeletedState).toHaveBeenCalledWith(
        "completed-under-test",
        true,
      );
    });
    ...
    expect(
      screen.getByText("Session hidden from default history."),
    ).toBeTruthy();
    ...
    await waitFor(() => {
      expect(mockSetCompletedSessionDeletedState).toHaveBeenNthCalledWith(
        2,
        "completed-under-test",
        false,
      );
    ...
    expect(screen.getByText("Delete")).toBeTruthy();
    expect(
      screen.getByText("Session restored to default history."),
    ).toBeTruthy();
  });
  ...
  it("shows feedback and preserves the delete label when delete persistence fails", async () => {
    const dataClient: CompletedSessionDetailDataClient = {
      ...
      appendCompletedSessionExerciseAsPlanned: jest
        .fn()
        .mockResolvedValue(undefined),
      setCompletedSessionDeletedState: jest
        .fn()
        .mockRejectedValue(new Error("Unable to update deleted state")),
    };
...
describe("CompletedSessionDetailRoute", () => {
  ...
  it("reads the session id from route params", async () => {
    ...
    expect(
      screen.getByTestId("completed-session-detail-screen").props.testID,
    ).toBe("completed-session-detail-screen");
  });
  ...
  it("redirects route intent=edit to the session view", async () => {
    ...
    expect(
      screen.getByTestId("completed-session-detail-edit-redirect"),
    ).toBeTruthy();
    await waitFor(() => {
  ...
  it("loads a persisted completed session by generated id via the default route data client", async () => {
    ...
    expect(mockLoadSessionSnapshotById).toHaveBeenCalledWith(
      generatedSessionId,
    );
  });
});
