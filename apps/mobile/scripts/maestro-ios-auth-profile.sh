...
# NOTE: the first-run log + remote round-trip flow
# (sync-first-run-log-and-roundtrip.yaml) used to run here; it now lives in its
# own dedicated lane — `npm run test:e2e:ios:sync` (maestro-ios-sync-e2e.sh) —
# the UI <-> real-backend sync e2e category. This lane stays focused on
# auth/profile + first-sync-gate surfaces.
