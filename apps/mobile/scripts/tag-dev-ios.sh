...
IPA_PATH="${1:-}"
EXPECTED_BUNDLE_ID="${EXPECTED_BUNDLE_ID:-com.phano.boga3.dev}"
...
TMP_DIR="$(mktemp -d /tmp/boga-dev-ipa-check.XXXXXX)"
...
if [[ "$BUNDLE_ID" != "$EXPECTED_BUNDLE_ID" ]]; then
  echo "Error: IPA bundle id does not match expected dev bundle id."
  echo "Expected: $EXPECTED_BUNDLE_ID"
  echo "Actual:   $BUNDLE_ID"
  echo
  echo "Set EXPECTED_BUNDLE_ID=<bundle-id> to override intentionally."
  exit 1
fi

TAG="dev-ios-v${VERSION}-b${BUILD}"
...
git tag -a "$TAG" -m "BoGa dev iOS development build v${VERSION} build ${BUILD}"
