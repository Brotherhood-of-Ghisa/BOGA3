 ...
 * reachability AND, with the connectivity fix in place, the real first cycle now
 * completes and dismisses the gate too fast to assert. 'clear' removes the pin so
 * the gate reflects real state again (paired with `bootstrap=complete` to
 ...
 * independent of the live scheduler / NetInfo and of how fast the real cycle
 * stamps the bootstrap flag. Pair 'clear' with `bootstrap=complete` to dismiss
 * the gate. A no-op for 'none'.
 */
