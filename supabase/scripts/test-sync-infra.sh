...
# auth-required-envelope) against THIS worktree's slot-isolated local Supabase,
# with zero manual environment setup. It:
...
#      SUPABASE_BRANCH_URL / SUPABASE_BRANCH_ANON_KEY the lane expects,
...
export SUPABASE_BRANCH_URL="${API_URL}"
export SUPABASE_BRANCH_ANON_KEY="${ANON_KEY}"
