...
- `npm test -- --runTestsByPath app/__tests__/completed-session-detail-screen.test.tsx app/__tests__/session-drafts-repository.test.ts --runInBand --no-watchman` — passed, 31 tests.
- `npm run typecheck` — passed.
- `npm run lint` — passed with existing warnings only.
- `npm run lint:ui-guardrails` — passed, 0 violations.
- `npm test -- --runInBand --no-watchman` — passed, 98 suites / 862 tests.
