...
11. In `session-recorder`, logged sets render as compact in-card text rows once they have displayable values; tapping the compact row turns it into inline editable inputs, and set numeric validation uses visual cues only (no inline validation text):
    ...
    - The former `Type` control is presented as right-side set quality in both compact and editable modes (`•`, `W-Up`, `RIR 0`, `RIR 1`, `RIR 2`); `•` maps to `null`, tapping cycles quality in the same order, and long-pressing opens the in-route modal picker with explicit options (`None`, `W-Up`, `RIR 0`, `RIR 1`, `RIR 2`). Newly imported unperformed planned target rows suppress this quality control while they show the initial `Log` / `Skip` decision controls; after the target is logged, edited into an actual, or skipped, the visible decision controls disappear and later log/skip status changes use the row swipe action. Quality is displayed and persisted separately from planned volume; by default, planned-row matched/modified classification compares prescribed volume only (`Weight` + `Reps`), not quality.
    ...
    - Active drafts preserve every set row, including fully blank and partial rows, with stable identity and order across input blur, tab/route navigation, and hydration. Blank or invalid reps remain incomplete; completion removes those rows only after the recorder's explicit cleanup confirmation. The `/sessions` active-session completion affordance returns to the recorder so it cannot bypass this cleanup flow.
    - Exercise cards start expanded and their title region toggles a volatile collapsed state. Collapsing dismisses the keyboard and closes editable rows or set-quality pickers inside that exercise without changing set data; replacing the exercise definition or appending/focusing a plan expands its target card.
    - A collapsed exercise shows its name plus `<valid performed sets> (<RIR 0 failures>)`. Blank, partial, invalid, skipped, and unperformed planned rows do not contribute to either count.
    - A collapsed active/completed-edit exercise additionally shows `New PR: <weight> kg × <reps> reps · est. 1RM <rounded kg> kg` only when its best valid current Wathan estimate strictly exceeds the maximum loaded completed-history estimate. Ties, first-ever exercises with no historical maximum, and loading/empty/error history states show no PR line.
