...
- Key states (high level):
  ...
  - per-exercise read-only recent block-history panel below tags and above set rows, with compact summary stats plus `<<` older / `>>` newer controls and inline loading/empty/error states
