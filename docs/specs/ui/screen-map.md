...
- Key states (high level):
  ...
  - per-exercise collapsed-by-default `Past Records` bar below tags and above set rows; tapping expands inline loading/empty/error states plus metric label / selected record date / live `Current` / `Max` rows for estimated `1RM`, volume, highest weight, and near-failure set count; left/right swipes change the selected historical record, and max values derive from the same loaded records
  - compact tap-to-edit set rows for normal and planned execution rows; appended historical/program targets support planned, matched, modified, skipped, and added states without replacing the existing recorder chrome; unperformed planned rows show `Log` / `Skip` without the right-side quality control until the user logs, edits, or skips the target; tapping a planned/skipped target to edit hydrates the actual fields from the plan, matched/modified classification defaults to prescribed volume rather than quality, and removable compact/editable rows delete via right-to-left swipe rather than visible row remove buttons
