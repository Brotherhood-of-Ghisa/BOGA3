...
- Required coverage should include the relevant subset of:
  ...
  - push-side FK closure preflight: an orphan dirty child (a `session_exercises` row missing its `sessions` parent, plus at least one layer-3 case — `exercise_sets` or `session_exercise_tags`) must be detected client-side and must NOT be sent to `sync_push` (RPC spy proves zero push calls), while valid parent+child graphs, clean-on-server parents, and independent valid rows are not falsely blocked; a present-but-quarantined parent must itself flag its child,
  - sync quarantine: an orphan detected by the preflight must be persisted to `sync_quarantine` (entity type/id, error code, diagnostic FK context, first/last seen, occurrence count), excluded from future push selection, and a valid independent dirty row beside it must still push and clear its dirty bit (one orphan must not wedge the backlog; the cycle converges rather than throwing); repeated detection must be an idempotent upsert (advance last-seen/count, never duplicate); the quarantine must survive a database reopen; quarantine must cascade to a child of a quarantined orphan; `getSyncStatus.blockedRowCount` must report blocked rows; the `sync.row_quarantined` and `sync.push_continued_after_quarantine` diagnostics must carry safe context only (opaque ids and structural metadata, never row payloads/names) and a logger failure must never prevent quarantine persistence or the continued push,
...
- Current frontend baseline suites for this policy include:
  ...
  - `apps/mobile/app/__tests__/sync-cycle-push-preflight.test.ts` (push-side FK closure preflight: orphan detection across layers, the present-but-quarantined-parent cascade flag, the RPC-not-called proof for orphan batches, the quarantine-not-wedge convergence with the orphan's dirty bit preserved, `sync.row_quarantined` safe-context logging and logger-failure isolation, and valid-graph topological push without false positives),
  - `apps/mobile/app/__tests__/sync-cycle-quarantine.test.ts` (sync quarantine: `quarantineRows` idempotent upsert / `readQuarantine` / `countQuarantinedRows`, `selectPushBatch` exclusion, and the whole-cycle proof that a valid row pushes beside a quarantined orphan, the quarantine persists across a database reopen, the `sync.push_continued_after_quarantine` event fires, a logger failure never blocks persistence/continuation, and a child of a quarantined orphan cascades into quarantine),
...
- Reinstall restore-parity coverage (sync v2): the dedicated `test:sync:reinstall-parity` mobile lane was **retired** in the sync v1 retirement — its target suite was deleted with the v1 code paths. Under the sync v2 model (push/pull RPC + per-layer cursor protocol) the same guarantee is proven by:
  ...
  - `apps/mobile/app/__tests__/sync/remote-gym-restore.test.ts` — fast-lane regression for the concrete bug "an empty local DB connecting to a remote account with gyms must restore those gyms": a fresh, never-bootstrapped in-memory store runs the real `runSyncCycle` against a stubbed RPC serving the remote layer-0 gyms, then asserts the local `gyms` table holds all of them clean and undeleted in the id+name shape the recorder gym picker will read. This covers the **data/sync** half. The **picker-visibility** half (the recorder still seeds its list from the hardcoded `SEEDED_LOCATIONS` constant and does not yet list the local `gyms` table) is owned by the still-planned `T-20260517-01-personal-gym-list-sync` and is deferred there; this test asserts the restored rows are present and queryable for that wiring.
