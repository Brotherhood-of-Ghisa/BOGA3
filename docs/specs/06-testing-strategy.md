...
| `npm run test:e2e:ios:smoke` | `scripts/maestro-run-lane.sh smoke` → runs `smoke-launch.yaml` with a `full` reset. Cold-launch + navigation smoke on the freshly-installed dev client (infra-free config): Today, the four tabs, then Train's Start opens the session view on an empty session. Captures `01-m26-today` … `05-m26-session-view-empty`. | iOS simulator + Metro + Maestro dev-client. **No** Supabase. | UI/runtime changes that need fresh real-simulator smoke evidence (see *iOS UI smoke policy*). Part of `boga test frontend`. |
| `npm run test:e2e:ios:data-smoke` | `scripts/maestro-run-lane.sh data-smoke` → runs `data-runtime-smoke.yaml` with a `data` reset. Validates real `expo-sqlite` migration + smoke write/read, backend-less starter-catalog seeding, a workout logged through Train, the session view's picker and the exercise page's logger, Finish, post-submit traversal through the completion presentation's Done action, and the session-write → Stats-exercise-list read-back (the only on-device check of that path: `stats-screen-ux` asserts the same list from fixture rows inserted straight into SQLite, which bypass the session screens). Captures `03-data-runtime-smoke-start`, `04-data-runtime-smoke-success`, `05-data-runtime-smoke-exercise-list`. | iOS simulator + Metro + Maestro dev-client. **No** Supabase. | See *iOS simulator data smoke policy* (bootstrap/migrations/drizzle/native-runtime changes). Part of `boga test frontend`. |
...
| `npm run test:e2e:ios:ui-regression` | `scripts/maestro-run-lane.sh ui-regression` (lane `ios-ui-regression`) — the **infra-free UI regression lane**: `stats-screen-ux` (empty state, exercise table + local sorting, 7→30-day rescale, muscle breakdown, and both history overlays with their metric inventories and Daily/Weekly switching), `session-completion-states-fixture` (one-PR, no-PR catalog-error, unmapped, safe-exit completion, and the completed edit through the session view: the completed session's `Edit`, a set changed on the exercise page, an invalid then valid Start with the autosave-paused notice, `Done`, and the read-back; then, on the `completion-two-prs` fixture, simultaneous multi-PR completion, session/working-set and exercise-volume summaries, image share preview plus retryable native-sheet launch/cancellation, the consolidated muscle-set breakdown, and Done), `settings-dev-wipe-local` (the developer wipe-local affordance: the card renders on a dev build, the wipe succeeds, and the app re-bootstraps to a usable data screen), and `exercise-catalogue` (DLM-T07: the catalogue grouped and flat, the Filters and row Actions sheets dismissed by the backdrop, the exercise editor empty, with validation errors, on its muscle list and filled, then Save and the `Exercise created.` notice; it leaves the stored list preference as it found it). All four reset their own data in-flow through the maestro-harness deep link, so they share **one** provisioned sim + Metro via `maestro-ios-run-flows.sh` with a `data` reset. | iOS simulator + Metro + Maestro dev-client. **No** Supabase. | UI/screen changes under `apps/mobile/app/**` or `components/**`; the session-insights, completion-presentation, Stats, Settings, or exercise catalogue/editor surfaces. Part of `boga test frontend`. |
| `npm run test:e2e:ios:exercise-page` | `scripts/maestro-run-lane.sh exercise-page` (lane `ios-exercise-page`) — the exercise page on its own `exercise-page` harness fixture: the accepted target's states (`V5-Quiet`, `V5-Records`, `V5-Last`, `V5-Effort`, `V5-Menu`, each captured), logging the current set with a changed effort and reps, `+ Add set`, Complete's discard warning (Cancel, then Complete), and a re-open proving the discarded planned sets are kept as not performed. Self-resets in-flow with `reset=data`, `data` reset. | iOS simulator + Metro + Maestro dev-client. **No** Supabase. | Changes to the exercise page (`app/session/**`, `components/exercise-page/**`, `src/session-recorder/**`) or the design-language primitives it composes. Part of `boga test frontend`. |
| `npm run test:e2e:ios:session-view` | `scripts/maestro-run-lane.sh session-view` (lane `ios-session-view`) — the **session view**: `session-view` (Train's Resume opens the session view; the summary card and one read-only card per exercise with its done count and 1RM record band; a card links to the exercise page; the ⋮ menu sheet; picking a gym from the Gym stat's sheet; the sheet's `Manage gyms` → the Gyms screen → add a gym → back to the reopened sheet → pick it; `+ Add exercise` through the exercise picker; Finish through one combined cleanup prompt for the empty set and exercises to the completion screen) and `session-view-abandon` (⋮ → Abandon session → confirmation → back on Train with no active session). Both seed with `?reset=data&fixture=session-view` and share one sim + Metro with a `data` reset. | iOS simulator + Metro + Maestro dev-client. **No** Supabase. | Changes to the session view (`app/session/**`, `components/session-view/**`), the Gyms screen (`app/gyms.tsx`, `components/gyms/**`), the shared picker/session model, or the active-session entries. Part of `boga test frontend`. |
...
3. **Every scroll whose next step taps or asserts the SAME element carries
   ...
   only for a target taller than the viewport, and say why in a comment. Both flags share one limit:
...
- Required smoke coverage: app launch visible state; the four tabs; Train's
  Start opening the session view; logged-out profile state; fixture-backed
  sign-in; signed-in profile state; username update; sign-out back to
  logged-out. Reset policy: `full reset` (smoke is the cold-start lane), then
...
- Purpose: validate runtime migration + smoke insert/read on real Expo iOS runtime
  ...
  also covered by `boga test frontend`). Reset policy: `data reset` plus
  `teleport` to Stats, then Train's Start and the session view; avoid
  `full reset` unless cold-install evidence is needed.
