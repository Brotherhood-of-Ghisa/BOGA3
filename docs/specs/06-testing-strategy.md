...
- Required coverage should include the relevant subset of:
  ...
  - push-side FK closure preflight: an orphan dirty child (a `session_exercises` row missing its `sessions` parent, plus at least one layer-3 case — `exercise_sets` or `session_exercise_tags`) must be detected client-side and must NOT be sent to `sync_push` (RPC spy proves zero push calls); the block must surface `LOCAL_FK_VIOLATION` (distinguishable from server `FK_VIOLATION`) with dirty bits left set, emit a sanitized `sync.push_fk_preflight_violation` diagnostic (opaque ids and structural metadata only, never row payloads/names) that survives a logger failure, while valid parent+child graphs, clean-on-server parents, and independent valid rows are not falsely blocked,
