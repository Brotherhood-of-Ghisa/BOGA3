...
- **Tests are not optional — no untested excuses.** Run every locally-runnable test/gate your change touches to GREEN before closeout, and put the evidenced run in the PR. A tool that "won't run" is a bootstrap gap (`npm install` + `./scripts/worktree-setup.sh`), NOT an unavailable tool — "command not found" / "infra unavailable" is never, by itself, a reason to skip. You have the full local toolchain (Supabase via `npx`, Maestro, iOS simulators). Never ship — and reviewers never approve — a PR that skips a runnable test with an excuse. The ONLY deferrable lanes are the genuinely cloud / branch-provisioned ones (`npm run test:sync:infra`, `npm run test:sync:reinstall-parity`) when `SUPABASE_BRANCH_URL` / `SUPABASE_BRANCH_ANON_KEY` are unset.
