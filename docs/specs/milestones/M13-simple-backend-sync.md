...
- On first enable:
  ...
  - merge with local state (local tie-break on equal timestamps),
...
12. `docs/tasks/complete/T-20260606-06-sync-restore-and-hardening-final-verification.md` - final verification that the sync-restore + FK-hardening series (tasks 01–05) is compatible with all pre-existing sync/data/scheduler suites and the backend Sync v2 contract/e2e gate; adds the fast-lane regression `app/__tests__/sync/remote-gym-restore.test.ts` proving an empty local DB + first authenticated sync restores remote gyms into local SQLite. The recorder picker-visibility half is deferred to `T-20260517-01-personal-gym-list-sync`. (`completed`)
