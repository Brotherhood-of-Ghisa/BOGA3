...
| 4 | Exercise page | ⬜ ready | New route behind the setting (3a); read it with `useNewScreensEnabled()`. Reuses `src/session-recorder/**`. Its own Maestro lane, which enables the setting first via the harness. Parallel with 5. |
| 5 | Session view | ⬜ ready | New route behind the same setting (3a). Its own Maestro lane, which enables the setting first via the harness. Parallel with 4. |
