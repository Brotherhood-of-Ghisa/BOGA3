...
| 7 | Close out | ⬜ blocked by the View Session restyle | Delete this plan and the build spec once every "Decisions already made" bullet is confirmed in `docs/specs/**` (most are in `ux-rules.md` §14a/§14b, `design-language.md` and screen-map 4b/21; check that "Back vs Complete are distinct exits" and "planned vs ad-hoc is not a mode" are in `ux-rules.md`). Delete `ui-tokens-additive.test.ts` and its mentions in `design-language` and `ux-rules` (and `app/__tests__/README.md`'s design-token policy); decide whether `uiColors` merges into `uiRoles`. Strip the "redesign step N" / "default since step 6a" wording from `screen-map.md`, `navigation-contract.md`, `ux-rules.md` §14a/§14b and `components-catalog.md`, and the provenance comments that still say "the recorder's …" in code and tests; Train's copy still says "the recorder" (`app/(tabs)/train.tsx`). Revisit the pending-primitives list in `components-catalog`. Optional: rename `src/session-recorder/` and `components/session-recorder/` (e.g. to `session/`); `scripts/triggers.tsv` cites `src/session-recorder/set-semantics.ts`. Blocked by the **View Session restyle** (`/completed-session/[sessionId]`, detail and completion presentation, its own task card, started after 6b-1 so it builds on the new screens' styling and components): until it ships, View Session is an old-styled screen on the legacy `uiColors`, so step 7 cannot retire them. |
