...
| 6b | Delete the old recorder | ⬜ blocked by 6a | Delete the old recorder route, its lanes and the setting itself. The old-recorder flows (`smoke-launch`, `data-runtime-smoke`, `exercise-block-history-fixture`, `session-completion-states-fixture` completed-edit, `sync-first-run-log-and-roundtrip`, `groups-link-exercise`) must move to the new screens or retire their recorder-only surfaces (live insights/share, completed-edit). Update `screen-map.md` + `navigation-contract.md`. Split into 6b-1/6b-2/6b-3 (task card `tasks/T-20260923-01-…`). **6b-1 built:** completed sessions are edited on the session view and exercise page (Start/End, `Done`), History / completed-session `Edit` / `intent=edit` open `/session/<id>`, `presentation=summary` removed. |
| 7 | Close out | ⬜ blocked by 6 | Delete this plan and the build spec; confirm every durable decision has graduated to `docs/specs/**`. Includes deleting `ui-tokens-additive.test.ts`, which exists only for the parallel period. Blocked also by the **View Session restyle** (`/completed-session/[sessionId]`, its own task card, started after 6b-1 so it builds on the new screens' styling and components): until it ships, View Session is an old-styled screen on the legacy `uiColors`, so step 7 cannot retire them. |
