...
| 5 | Session view | ✅ shipped | `app/session/[sessionId]/index.tsx` behind the setting; every active-session entry (Today, Train, Sessions, completed-session append) goes there when it is on (`activeSessionHref`). The recorder's pure session model, submit cleanup, completion write and exercise picker moved to `src/session-recorder/session-model.ts`, `session-lifecycle.ts` and `components/session-recorder/exercise-picker.tsx`, so both recorders write through one copy. Card rows keep the mini legends, and every figure takes its row's colour and weight — no per-column `best` bold, only a record 1RM in `record` (design-language §5's `best` is not used on this card); the tab bar stays (it is the way out). Finish prompts are native alerts with the recorder's copy; Abandon confirms. Lane `ios-session-view`. Cards link to step 4's exercise page. Contract graduated to `screen-map.md` / `navigation-contract.md` / `ux-rules.md` §14b. |
| 6a | Default on | ✅ shipped | `DEFAULT_NEW_SCREENS_ENABLED = true`, same `.v1` key, so a stored Off still wins. Settings copy now offers Off as the way back to the recorder. The new-screen flows dropped `newScreens=on` so they prove the default path; `settings-new-screens-toggle` proves `newScreens=off` and that `reset=data` restores On. Old-recorder flows are untouched: `teleport=session-recorder` ignores the setting. |
| 6b | Delete the old recorder | ✅ shipped | Split into three PRs (task card deleted with 6b-3; git history keeps it). **6b-1** (#330): completed sessions are edited on the session view and exercise page (Start/End, `Done`); History / completed-session `Edit` / `intent=edit` open `/session/<id>`; `presentation=summary` removed. **6b-2** (#329): the gym sheet's `Nearby` suggestion (never preselected), `Manage gyms`, the `/gyms` screen (soft-delete archive) and More → Tools `Gyms`; the exercise page's ⋮ `Link to group exercise…`. **6b-3**: `/session-recorder`, the `New exercise & session screens` setting, `activeSessionHref`, the recorder teleport and `newScreens=` harness param deleted; tag editing (D4) and the live muscle-load row (D5) dropped; `source=session`; picker coverage moved to `exercise-picker.test.tsx`; the old-recorder flows now run through Train → session view → exercise page, and the completion tail moved to `session-completion-states-fixture` (`completion-two-prs` fixture). Contract graduated to `screen-map.md`, `navigation-contract.md`, `ux-rules.md`, `components-catalog.md` and specs 02/03/05/06/08/09/11. |
| 7 | Close out | ⬜ ready | Delete this plan and the build spec once every "Decisions already made" bullet is confirmed in `docs/specs/**` (most are in `ux-rules.md` §14a/§14b, `design-language.md` and screen-map 4b/21; check that "Back vs Complete are distinct exits" and "planned vs ad-hoc is not a mode" are in `ux-rules.md`). Delete `ui-tokens-additive.test.ts` and its mentions in `design-language` and `ux-rules` (and `app/__tests__/README.md`'s design-token policy); decide whether `uiColors` merges into `uiRoles`. Strip the "redesign step N" / "default since step 6a" wording from `screen-map.md`, `navigation-contract.md`, `ux-rules.md` §14a/§14b and `components-catalog.md`, and the provenance comments that still say "the recorder's …" in code and tests; Train's copy still says "the recorder" (`app/(tabs)/train.tsx`). Revisit the pending-primitives list in `components-catalog`. Optional: rename `src/session-recorder/` and `components/session-recorder/` (e.g. to `session/`); `scripts/triggers.tsv` cites `src/session-recorder/set-semantics.ts`. The **View Session restyle** is done (task `T-20260923-02`, PRs A–C): the completed-session detail, its completion presentation and the group session view use the design language. Today, Train, Progress, More, groups and Settings still use `uiColors`, so merging it into `uiRoles` is an app-wide migration, not a close-out item. |
