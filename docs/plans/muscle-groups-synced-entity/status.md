...
## 2026-06-07 — iteration 11
- Merged this iter: t10 (PR #176), human-merged. Coordinator rebased onto origin/main @ d3930c9. Hand-off verified: ZERO plan refs in supabase/ (hygiene clean). Deviations log updated (t10).
- MAJOR: investigated unrelated PR #148 (merged earlier). It ALREADY shipped boot FK enforcement (bootstrap.ts: configureForeignKeyEnforcement at conn-open + post-seed foreign_key_check) + FK error classification/quarantine/preflight/scheduler-semantics (44 files). VERIFIED merged t2/t4/t6/t9 survived #148/#175 intact (no clobber, no conflict markers). Plan outcome 6 FK-pragma = pre-satisfied by #148.
- Re-scoped t5: added a coordinator pointer marker to tasks/t5.md — DO NOT re-add the FK pragma; reconcile with #148's bootstrap structure; verify FK enforcement holds under the muscle_groups dirty seed. Core t5 work (dirty seed via seedSystemExerciseCatalog, remove standalone seed, generic account-wipe, comment fixes) stands.
- t7 + tFINAL flagged for #148 reconciliation when they come (FK-harness default, false-FK-comment fixes, PO6/PO8 assertions).
- Dispatched: t5 (builder, backend lane, solo per serialization rule).
- Stuck: none.

## 2026-06-07 — iteration 12
- t5 PR #180 built (builder ae3e643). ALL gates green: fast (90 suites/802 tests), test:handles, backend clean-reset (drift 9 entities exit 0), iOS DATA-SMOKE passed. #148 FK pragma: builder verified existing placement works under the dirty muscle_groups seed (baseline migrations CREATE-only, no rebuilds; Layer-0 parents seeded before Layer-1; post-seed foreign_key_check passes) — did NOT re-add. Builder deleted muscle-group-bootstrap-idempotent.test.ts (only tested the removed standalone seed) + reconciled other broken tests. 194+/443−.
- PLAN-OUTCOME CONFLICT (external): #148 commit a20fbbf added a 2nd client migration m0001 (`0001_dapper_vision.sql`, `sync_quarantine` table) and updated domain-schema-migrations.test.ts to assert journal length 2 / ['m0000','m0001']. This invalidates plan **outcome 4's literal** "journal length 1 / ['m0000'] / no incremental migration". The muscle_groups SPIRIT holds (t3 kept muscle_groups in m0000; m0001 is unrelated quarantine). NEEDS user sign-off to re-scope outcome 4 + tFINAL PO4 to assert the spirit (muscle_groups lives in m0000, adds no migration of its own) instead of the now-false "single migration". Only affects tFINAL (last task) — not urgent.
- t7 NOTE: its card lists muscle-group-bootstrap-idempotent.test.ts as t7 territory, but t5 already deleted it (had to, to keep gates green). Adjust t7 brief when dispatched.
- Dispatched: t5 reviewer (report-to-coordinator).
- Reviewed: t5 PR #180 → `Verdict: APPROVED` (reviewer a32c5bf, report-to-coordinator). Confirmed all 6 outcomes, #148 FK reconciliation sound (single pragma, no weakening, Layer-0-before-Layer-1 seed order → post-seed foreign_key_check passes), NO drizzle/baseline touched (m0001 absent from t5 diff), deleted muscle-group-bootstrap-idempotent.test.ts justified (only tested removed standalone seed), reconciled tests faithful, hygiene clean. One non-blocking cosmetic nit (repeated pragma phrase in a comment) — let go. t5 OPEN + ready to merge.
- Pending user decisions: (1) merge t5 #180; (2) re-scope outcome 4 + tFINAL PO4 to the muscle_groups SPIRIT (m0000 carries muscle_groups, no muscle_groups migration; m0001 is unrelated #148 quarantine) since the literal "journal length 1" is now impossible without reverting #148.

## 2026-06-07 — iteration 13
- USER DECISIONS applied: t5 merged; outcome-4 re-scoped to the spirit (plan.md outcome 4 + tFINAL PO4 edited). t11 folded in earlier.
- Synced to t5-merged main @ 20954a3. t5 hand-off verified (standalone seed gone, generic wipe). t7 recon: harness still FK-off-by-default (flip needed); muscle-group-bootstrap-idempotent.test.ts already deleted by t5; cycle/dirty-bit suites partly cover muscle_groups; t5 fixed some false FK comments. Added re-scope pointer marker to t7.md.
- Dispatched (parallel, disjoint): t11 reviewer (#181) + t7 builder (#182, re-scoped).
- Reviewed: t11 #181 → `Verdict: APPROVED` (reviewer abe9d65) — cross-owner probe correct (A-actor vs B-owned rows); backend green. t7 #182 → `Verdict: APPROVED` (reviewer a5633707) — FK-on default flip clean (ZERO tests silenced to FK-off; flip surfaced zero insertion-order regressions), +3 muscle_groups cases, fk-integrity reconciled to topo framing, false comments swept, stayed tests-only (dev-affordances.ts change is comment-only). Both OPEN + ready to merge.
- NEW PRODUCTION BUG surfaced by t7 builder + coordinator-verified: `apps/mobile/src/sync/fk-graph.ts` (push FK-closure preflight from #148) omits the `exercise_muscle_mappings.muscle_group_id → muscle_groups` edge from `FK_EDGES` (lines 63-65) + `FK_PARENT_TABLES`, with a now-false "muscle_groups ... never pushed" comment (lines 21-24). muscle_groups is now a pushed synced parent → orphan mappings escape the local preflight and get server-rejected. Same consumer-parity class as t9/t11 but PRODUCTION correctness. Builder spawned a chip; surfacing fold-in decision (candidate t12, + a sweep for any other stragglers).
- Dispatched: none new (awaiting t11+t7 merge + t12 decision).

## 2026-06-08 — iteration 14
- Merged: t7 (#182) + t11 (#181). Synced @ 08776bc. Hand-offs verified (harness FK-on default; rls probes muscle_groups). Deviations log caught up (t5/t7/t11 + outcome-4 rescope). Created+wired t12.
- Dispatched (parallel, disjoint, fast-lane): t8 (docs #184) + t12 (fk-graph + sweep #183).
- t12 #183 → built + `Verdict: APPROVED` (reviewer ab7863e): fk-graph SYNCABLE_FK_GRAPH gains muscle_group_id→muscle_groups edge (required:true) + PARENT_TABLES entry + 4 preflight test cases; sweep confirmed fk-graph was the ONLY apps/mobile/src straggler (account-wipe/sync-status already say nine). Ready to merge.
- t8 #184 → built (builder a77a5cf): both specs reframed to synced-entity reality (9 entities, real FK, boot FK enforcement, synced-parent-FK invariant, drift exemption dropped). t12's sweep flagged docs/specs/05-data-model.md still said "client-only taxonomy" on PRE-t8 main — t8 appears to fix it (line 277 "no longer exempt — one of the nine"); dispatched t8 reviewer to CONFIRM no surviving wrong client-only/never-synced claim about muscle_groups (esp. the ~line 62 "Never synced and FK-free"). t8 builder's doc-straggler chip superseded by t8 itself.
- After t8+t12 merge → tFINAL is the ONLY remaining task (needs t1-t12 all merged). Then audit.

## 2026-06-08 — iteration 15
- t12 #183 → APPROVED (reviewer ab7863e), ready to merge. t8 #184 → reviewer aa3e863 `Verdict: CHANGES_REQUESTED` (1 item: docs said FK pragma "after migrations"; as-built #148 = connection-open). Re-dispatched t8 (builder a6af9f7) → fixed both 05-data-model.md locations to connection-open reality; gates green; pushed to same branch (head 1ccaf0b), mergeable. Coordinator confirmed the fix on the branch. t8 substantively approved, ready to merge (CI finishing).
- PLAN.MD RESET incident: plan.md + t5/t7/tFINAL cards were reset to pre-execution state + staged in my worktree (flagged intentional). My commits + status.md retained full bookkeeping. USER chose RESTORE. Restored from HEAD; reconciled gaps: re-added t5/t7/t11 + t8/t12 + outcome-4/6 deviations entries (HEAD was missing the uncommitted iter-14 ones); fixed stale outcome 6 + tFINAL PO6 "after migrations" → connection-open (matching #148 as-built + the t8 doc fix). Committing reconciled bookkeeping.
- REMINDER (still pending): the plan dir on origin/main is the ORIGINAL #167 version (no t9-t12 cards, no deviations). MUST push the full current plan dir to main before dispatching the audit (audit reads from main). Plan: after tFINAL merges, open a coordinator bookkeeping commit/PR with the plan dir, merge, then dispatch audit.
- Dispatched: none new. Awaiting t8+t12 merge.

## 2026-06-08 — iteration 16
- Merged: t8 (#184) + t12 (#183). All t1-t12 now merged. Synced @ 859db55. Hand-offs verified (t8 docs connection-open wording; t12 fk-graph muscle_groups edge).
- MAO SKILL UPDATED (user request): SKILL.md + references/deviations-and-progress.md now codify that the coordinator owns the plan dir and keeps it current ON the integration branch via its own self-merged `[coordinator] <plan> bookkeeping` PR — the single carve-out to "coordinator never merges" (plan dir only; task/design/build/audit PRs still human-merged).
- APPLIED new policy: pushed + SELF-MERGED bookkeeping PR #185 (plan dir only, +446/-14: t9-t12 cards, status.md, deviations log, outcome-4/6 rescope, tFINAL t1-t12). main plan dir now current @ 9a91ee8. (Resolves the stale-plan-dir-on-main problem ahead of tFINAL + audit.)
- Dispatched: tFINAL (builder, backend + data-smoke lanes, solo) — the final test card asserting PO1-PO9 end-to-end, centerpiece the anti-brick reinstall round-trip under FK enforcement.
- After tFINAL merges → push final bookkeeping to main → dispatch mao-audit.
- Reviewed: tFINAL #188 → `Verdict: APPROVED` (reviewer a589e2a, report-to-coordinator). Every PO1-PO9 has a non-vacuous test; central PO8 anti-brick round-trip genuine (real runSyncCycle FK-on, negative control = orphan@L1 → fk-violation); PO4/PO6 re-scopes honored (no length-1 assertion; FK live-before-seed via connection-open); test-only (sole non-test change = 1-line e2e script registration); gates green incl. iOS data-smoke 1/1. tFINAL OPEN + ready to merge — the LAST task.
- NEXT: human merges tFINAL → coordinator pushes final bookkeeping to main (self-merged `[coordinator]` PR per new policy) → dispatch mao-audit (verifies PO coverage, deviations log completeness, hand-offs, gates on main; deletes plan dir on PASS).

## 2026-06-08 — iteration 17 (close-out)
- Merged: tFINAL (#188). ALL 13 cards (t1-t12 + tFINAL) now merged. Synced @ 7a469ea (also absorbed unrelated #186 heatmap UX). tFINAL hand-off verified: all 4 test artifacts + e2e registration present on main. Deviations log completed (t8/t12/tFINAL entries added).
- Landing final bookkeeping to main (self-merged `[coordinator]` PR) so the audit reads the complete plan dir.
- Dispatching mao-audit with an EXTRA user-requested mandate: verify IN DEPTH whether the sync-specific documentation (docs/specs/05-data-model.md, docs/specs/tech/sync-v2-server-contract.md, and any other sync docs) is fully up to date vs the as-built code — not just that t8's edits landed, but a thorough doc-vs-code reconciliation across the whole sync surface (incl. #148 FK-infra interactions).
- USER DECISION: fold fk-graph bug in as **t12** ("fix + sweep"). Created tasks/t12.md (add muscle_group_id→muscle_groups edge to FK_EDGES + FK_PARENT_TABLES + fix false comment + preflight test that orphan mapping is caught locally; AND sweep apps/mobile/src for other client-only/8-entity stragglers). Wired DAG `t7 → t12 → tFINAL`; added to tasks; tFINAL inputs → t1-t12. (Builder chip superseded.)
- Remaining after t11+t7 merge: t8 (docs; needs t5✓+t6✓+t7) ∥ t12 (needs t7) — disjoint, both fast-lane, dispatch in parallel. Then tFINAL (needs ALL: t1-t12). Plan now 12 build/design tasks + tFINAL (grew from 9 via t9/t10/t11/t12, all real consumer-parity/fallout gaps).
- REMINDER: push accumulated coordinator bookkeeping (new cards t9-t12, plan.md DAG/deviations/outcome-4, status.md) to main before the audit — currently only in coordinator worktree; audit reads from main.
