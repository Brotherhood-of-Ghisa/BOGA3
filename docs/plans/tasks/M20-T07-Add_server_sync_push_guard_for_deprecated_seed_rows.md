---
task_id: M20-T07-Add_server_sync_push_guard_for_deprecated_seed_rows
milestone_id: "M20"
status: planned
ui_impact: "yes"
...
# M20-T07-Add_server_sync_push_guard_for_deprecated_seed_rows

## Task metadata

- Task ID: M20-T07-Add_server_sync_push_guard_for_deprecated_seed_rows
...
- Known stale references or assumptions: old app builds can still seed the pre-M20 bundle locally; this task prevents exact known deprecated seed IDs from persisting remotely as active rows.
- Optional helper command:
  - `./scripts/task-bootstrap.sh docs/plans/tasks/M20-T07-Add_server_sync_push_guard_for_deprecated_seed_rows.md`

## Objective

Prevent old app builds from re-populating the remote Sync v2 mirror with exact
known deprecated bundled seed rows after M20 cleanup, while giving supported app
...
- Add a `sync_push` guard for the finalized M20 suppressed exercise definition IDs and their exact seed mapping IDs.
...
1. Old client pushes known deprecated seeds:
   - Trigger: an old app build signs into an empty remote account and pushes its pre-M20 seed bundle.
...
1. `sync_push` cannot persist exact known deprecated M20 seed definition or mapping IDs as active remote rows after the guard deploys.
...
- Constraints/assumptions:
  - Use exact finalized M20 suppressed seed IDs from `M20-T01`; do not use broad name matching.
