---
task_id: M20-T04-Add_catalog_prune_regression_tests
milestone_id: "M20"
status: planned
ui_impact: "no"
...
# M20-T04-Add_catalog_prune_regression_tests

## Task metadata

- Task ID: M20-T04-Add_catalog_prune_regression_tests
...
- Optional helper command:
  - `./scripts/task-bootstrap.sh docs/plans/tasks/M20-T04-Add_catalog_prune_regression_tests.md`
...
- Assert the finalized M20 exercise-definition and mapping counts recorded by
  `M20-T01`.
...
- Cover the fallback stale-catalog error classification and user-visible
  update-required sync state if `M20-T07` adds that rejection path.
...
1. Tests fail against the old long catalog or unguarded `sync_push` behavior and
   pass against the M20 pruned/guarded catalog.
...
4. Existing seed-count assertions are updated to the finalized M20 counts.
...
- Planned files/areas allowed to change:
  ...
  - `apps/mobile/src/sync/**` UI-state tests only if stale-catalog
    classification is added by `M20-T07`
