---
task_id: M20-T03-Add_remote_catalog_cleanup_migration
milestone_id: "M20"
status: planned
ui_impact: "no"
areas: "backend|docs"
...
# M20-T03-Add_remote_catalog_cleanup_migration

## Task metadata

- Task ID: M20-T03-Add_remote_catalog_cleanup_migration
...
- Known stale references or assumptions: old app builds can still seed the old
  bundle locally; task `M20-T07` owns the server-side guard that prevents those
  rows from persisting remotely as active rows.
- Optional helper command:
  - `./scripts/task-bootstrap.sh docs/plans/tasks/M20-T03-Add_remote_catalog_cleanup_migration.md`
...
- Any change to sync RPC wire contract or future `sync_push` stale-seed guard
  behavior; `M20-T07` owns that slice.
...
5. Cleanup migration can be paired with the `M20-T07` guard without conflicting
