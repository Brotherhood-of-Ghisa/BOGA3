---
task_id: M20-T01-Prune_starter_catalog_seed_bundle
milestone_id: "M20"
status: planned
ui_impact: "no"
areas: "frontend|docs"
...
gates_slow: "N/A"
docs_touched: "docs/plans/milestones/M20-prune-starter-exercise-catalog.md"
---

# M20-T01-Prune_starter_catalog_seed_bundle

## Task metadata

- Task ID: M20-T01-Prune_starter_catalog_seed_bundle
...
- Optional helper command:
  - `./scripts/task-bootstrap.sh docs/plans/tasks/M20-T01-Prune_starter_catalog_seed_bundle.md`

## Objective

Shrink the bundled starter exercise seed bundle to a curated keep set while
keeping seed validation, mapping coverage, documentation coverage, and the M20
...
- Audit the current `SYSTEM_EXERCISE_DEFINITION_SEEDS` and produce the final
  M20 keep/suppress table in the milestone before pruning.
- Prune `SYSTEM_EXERCISE_DEFINITION_SEEDS` to the finalized M20 keep set.
...
- Rename kept bundled exercise names to the M20 singular/equipment-specific
  display names.
...
- UI Impact?: `no`
- This task changes seed data only; visible catalog behavior is verified in a later M20 task.

## Acceptance criteria

1. The seed bundle validates with no duplicate exercise definition IDs/names.
2. The seed summary reports the finalized M20 exercise and mapping counts, and
...
- Planned docs/spec files to update and why:
  - `docs/plans/milestones/M20-prune-starter-exercise-catalog.md` - update task status/details if the finalized keep set changes during implementation.
