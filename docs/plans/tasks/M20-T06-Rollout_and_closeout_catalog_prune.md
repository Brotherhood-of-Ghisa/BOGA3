---
task_id: M20-T06-Rollout_and_closeout_catalog_prune
milestone_id: "M20"
status: planned
ui_impact: "no"
...
docs_touched: "docs/plans/milestones/M20-prune-starter-exercise-catalog.md, docs/specs/02-quality-and-test-gates.md"
---

# M20-T06-Rollout_and_closeout_catalog_prune

## Task metadata

- Task ID: M20-T06-Rollout_and_closeout_catalog_prune
- Title: Rollout and closeout catalog prune
- Status: `planned`
- File location rule:
  - author active cards in `docs/plans/tasks/<task-id>.md`
  - move the file to `docs/plans/tasks/<task-id>.md` when `Status` becomes `completed` or `outdated`
...
- Milestone spec: `docs/plans/milestones/M20-prune-starter-exercise-catalog.md`
...
- Parent refs opened in this session:
  - `docs/plans/milestones/M20-prune-starter-exercise-catalog.md`
...
- Known stale references or assumptions: old app builds can still seed the
  previous long catalog locally, but `M20-T07` should prevent known deprecated
  seed rows from persisting remotely as active rows after the guard deploys.
- Optional helper command:
  - `./scripts/task-bootstrap.sh docs/plans/tasks/M20-T06-Rollout_and_closeout_catalog_prune.md`

## Objective

Close the M20 milestone with verified gates, documented rollout notes, and a
...
- Run `./boga test for --diff <range>` and all required gates for the final M20 diff.
...
- Move completed M20 task cards to `docs/plans/tasks/` as appropriate.
- Update the M20 milestone completion note and status when all slices are complete.

### Out of scope

- Implementing any remaining M20 behavior not already completed by prior tasks.
...
1. All M20 implementation task cards are either completed or explicitly blocked/outdated with rationale.
...
4. No source-of-truth docs remain stale for behavior changed during M20.
5. The final diff contains no unplanned implementation scope.

## Docs touched

- Planned docs/spec files to update and why:
  - `docs/plans/milestones/M20-prune-starter-exercise-catalog.md` - closeout status and completion note.
  - `docs/specs/02-quality-and-test-gates.md` - update only if lane registry/triggers changed.
  - Other project-level docs only if earlier M20 tasks changed canonical behavior without updating them.
...
- Slow-gate triggers: backend/frontend/sync changes made by M20.
...
- If `Status = completed` or `outdated`, move the task card to `docs/plans/tasks/` and update affected references in the same session.
